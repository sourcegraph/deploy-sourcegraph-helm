...
- Corrected the external object storage examples to configure the shared store for frontend, worker, precise code intel, gitserver, and searcher, and the separate legacy precise upload store used by syntactic code intel, including credentials or workload service accounts as required.
